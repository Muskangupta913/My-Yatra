...
function fetchPackages(selectedPrice, selectedDuration, selectedType) {
  $.ajax({
    url: '{{ url("/filter-tour-packages", $newSlug) }}',  // Generate correct URL
    method: 'GET',
    data: {
        price: selectedPrice,
    ...
    headers: {
            ...
            success: function(response) {
       $('#package-type-list').empty();  // Clear the package list container
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="card shadow card-body p-0">
                            <a href="/tour-category/${response.newSlug}/${item.slug}" class="text-decoration-none text-dark">
                                <img src="/uploads/packages/${item.photo}" width="100%" style="height:250px; object-fit:cover;" alt="Package Image">
                            </a>
                            <div class="package-desc py-3 px-2">
                                ...
                                <div class="price-offer d-flex justify-content-between align-items-center">
                                    ...
                                    <button class="btn btn-danger mx-2 btn-lg rounded-0 booknow" data-id="${item.id}  id="booknow" data-bs-toggle="modal" data-bs-target="#bookingModal">Bookk Now</button>
                                </div>
                            </div>
                        </div>
                    </div>
                `;
    // Append the generated HTML to your DOM
