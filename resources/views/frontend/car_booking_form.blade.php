    ...
    <div class="search-summary bg-white p-4 rounded-lg shadow-sm mb-6">
        ...
        <div class="flex justify-between items-center text-center flex-wrap gap-4 md:gap-6" id="search-summary">
            <div class="flex gap-2">
                ...
                <span>Date: <strong id="pickup-date">03/20/2025</strong> | </span>
        ...
        <div class="booking-grid">
            ...
            <section class="booking-form">
                <form action="{{ route('car.payment') }}" method="POST" id="bookingForm">
                    ...
                    <input type="hidden" name="car_base_fare" id="car-base-fare-input" value="">
                    ...
                    <fieldset class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="pickup_address" name="pickup_address"
                                class="form-control @error('pickup_address') border-red-500 @enderror" rows="3" required></textarea>
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="drop_address" name="drop_address" class="form-control @error('drop_address') border-red-500 @enderror"
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" id="booking_date" name="booking_date"
                                class="form-control @error('booking_date') border-red-500 @enderror" required>
                            @error('booking_date')
                                <span class="error-message">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-group">
                        <label class="inline-flex items-center">
                            ...
                            <span class="ml-2">I agree to the <a href="#" class="text-primary-600">terms and
                                    conditions</a></span>
                        </label>
                        @error('terms')
                            <span class="error-message">{{ $message }}</span>
            ...
            <aside class="car-details">
                ...
                <div class="car-info-grid">
                    ...
                    <div class="car-info-item" style="display: none;">
                    ...
                    <div class="car-info-item" style="display: none;">
                        ...
                        <span>Base Fare: <strong id="car-base-fare"></strong></span>
                    </div>
                </div>
            </aside>
        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const validators = {
                ...
                pickup_address: (value) => value.trim().length >= 10 ||
                    'Please enter a detailed pickup address',
            ...
            async function checkBalance() {
                try {
                    ...
                    if (availableBalance < carPrice) {
                        throw new Error(
                            `Insufficient balance. Available: ₹${availableBalance}, Required: ₹${carPrice}`);
                    }
            ...
            if (carData && Object.keys(carData).length > 0) {
                ...
                document.getElementById("car-base-fare-input").value = carData.baseFare || "0.00";
                ...
                document.getElementById("car-price").textContent = "₹" + parseFloat(carData.totalAmount || 0)
                    .toFixed(2);
                ...
                document.getElementById("car-base-fare").textContent = "₹" + parseFloat(carData.baseFare || 0)
                    .toFixed(2); // Correct Base Fare
            }
            ...
            // Populate hidden booking date field
            const pickupDateElement = document.getElementById("pickup-date");
            const hiddenBookingDateInput = document.getElementById("hidden-booking-date");
            if (pickupDateElement && hiddenBookingDateInput) {
                hiddenBookingDateInput.value = pickupDateElement.textContent.trim();
            }

            // Populate booking date input field with the correct format
            const bookingDateInput = document.getElementById("booking_date");
            if (pickupDateElement && bookingDateInput) {
                const pickupDate = pickupDateElement.textContent.trim(); // Format: MM/DD/YYYY
                const [month, day, year] = pickupDate.split('/');
                bookingDateInput.value =
                    `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`; // Format: YYYY-MM-DD
            }

            // Ensure the displayed date in the input field matches the desired format
            if (bookingDateInput) {
                const [year, month, day] = bookingDateInput.value.split('-');
                bookingDateInput.value = `${day}-${month}-${year}`; // Format: DD-MM-YYYY
            }
